...
@section('content')
    <div class="dashboard-main-body">
        ...
        <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
            <div>
                <h1 class="fw-semibold mb-4 h6 text-primary-light">Blogs</h1>
                <div>
                    <a href="{{ route('dashboard') }}"
                        class="text-secondary-light hover-text-primary hover-underline">Dashboard</a>
                    <span class="text-secondary-light">/ Blogs</span>
                </div>
            </div>
            <a href="{{ route('blogs.create') }}" class="btn btn-primary-600 d-flex align-items-center gap-6">
                <i class="ri-add-large-line"></i> Write New Blog
            </a>
        </div>
                                    ...
                                    <tr>
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center gap-12">
                                                ...
                                                <div>
                                                    <p class="fw-medium mb-0">{{ $blog->title }}</p>
                                                    <small
                                                        class="text-muted">{{ Str::limit($blog->short_description, 50) }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td><span
                                                class="badge bg-light text-dark border">{{ $blog->category ?: 'Uncategorized' }}</span>
                                        </td>
                                        <td>{{ $blog->author ?: 'Admin' }}</td>
                                        <td>{{ $blog->created_at->format('d M Y') }}</td>
                                        <td>
                                            <form action="{{ route('blogs.toggle-status', $blog) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                <button type="submit"
                                                    class="badge bg-{{ $blog->status ? 'success' : 'danger' }} bg-opacity-10 text-{{ $blog->status ? 'success' : 'danger' }} border border-{{ $blog->status ? 'success' : 'danger' }} px-2 py-1 fs-13 fw-medium">
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end border p-2">
                                                    ...
                                                    <li><button type="button"
                                                            class="dropdown-item rounded text-secondary-light bg-hover-neutral-200 text-hover-neutral-900 d-flex align-items-center gap-2 py-6"
                                                            onclick="confirmDelete({{ $blog->id }}, '{{ route('blogs.destroy', $blog) }}')">
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body text-center p-4">
                    ...
                    <h5 class="fw-bold mb-1">Delete Blog?</h5>
